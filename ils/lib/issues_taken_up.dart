...
import 'info_issues_taken.dart';


class IssuesTakenUpScreen extends StatefulWidget {
  var employee;
  @override
  State<StatefulWidget> createState() {
    return _IssuesTakenUpScreenState(employee);
  }
  IssuesTakenUpScreen(this.employee);
}

class _IssuesTakenUpScreenState extends State<IssuesTakenUpScreen> {
  var employee;
  ...
  _IssuesTakenUpScreenState(this.employee){
    _getAllYourTakenIssues(employee);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      //This scaffold is the basic screen where the widgets are placed
        ...
        appBar: _appBar("Issues taken up"),
  ...
  Widget _showTimeStamp(List <Issue> arrayOfIssues, int index, double font) {
    return Row(
        children: [
          Icon(Icons.access_time, size: font, color: Styles.iconColorGray,),
          Text(_timeStampTrim(arrayOfIssues[index].Issue_Logged_Timestamp)[1] +
              "     ", style: TextStyle(fontSize: font,
              color: Styles.iconColorGray,
              fontFamily: 'Roboto-Regular'),),
          Icon(Icons.date_range, size: font, color: Styles.iconColorGray,),
          Text(_timeStampTrim(arrayOfIssues[index].Issue_Logged_Timestamp)[0] +
              "     ", style: TextStyle(fontSize: font,
              color: Styles.iconColorGray,
              fontFamily: 'Roboto-Regular'),)
        ]
            ...
            child: Container(
                ...
                child: ListTile(
                  onTap: (){
                    Navigator.push(
                      context,
                      MaterialPageRoute(builder: (context) => IssueDetails(arrayOfIssues[index].Issue_no,this.employee, "Update")),
                    );
                    },
                  contentPadding:
  ...
  _getAllYourTakenIssues(String employee) {
    AppServices.getYourTakenIssues(employee).then((allIssuesEver) {
      setState(() {
